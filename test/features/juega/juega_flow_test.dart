...
void main() {
  ...
  setUp(() {
    ...
    testUnidad = const Unidad(
      ...
      cancionPulso: CancionPulso(
        titulo:
            LocalizedString(gl: 'As ondas da ría', es: 'Las olas de la ría'),
      ...
      vocabulario: [
        VocabularioItem(
          ...
          definicionBreve:
              LocalizedString(gl: 'Líquido do mar', es: 'Líquido del mar'),
          imagenAsset: 'assets/images/voc/auga.png',
          audioAsset: LocalizedString(
              gl: 'assets/audio/auga.mp3', es: 'assets/audio/agua.mp3'),
        ),
      ],
      preguntas: [
        PreguntaNivel(
          ...
          enunciado: LocalizedString(
              gl: 'Onde está o barco?', es: '¿Dónde está el barco?'),
          respuestaSugerida:
              LocalizedString(gl: 'Sinala co dedo', es: 'Señala con el dedo'),
          consejoDocente: LocalizedString(
              gl: 'Acompañar o xesto', es: 'Acompañar el gesto'),
        ),
        PreguntaNivel(
          nivel: 2,
          enunciado:
              LocalizedString(gl: 'Que fai a auga?', es: '¿Qué hace el agua?'),
          respuestaSugerida:
              LocalizedString(gl: 'Chof, chof!', es: '¡Chof, chof!'),
          consejoDocente: LocalizedString(
              gl: 'Facer onomatopeias', es: 'Hacer onomatopeyas'),
        ),
        PreguntaNivel(
          nivel: 3,
          enunciado: LocalizedString(
              gl: 'Por que flotan os barcos?',
              es: '¿Por qué flotan los barcos?'),
          respuestaSugerida: LocalizedString(
              gl: 'Porque son lixeiros', es: 'Porque son ligeros'),
          consejoDocente: LocalizedString(
              gl: 'Probar na conca', es: 'Probar en el barreño'),
        ),
      ],
      exploracion: ExploracionSensorial(
        titulo:
            LocalizedString(gl: 'Xogando coa auga', es: 'Jugando con el agua'),
        materiales: [
          LocalizedString(gl: 'Conchas grandes', es: 'Conchas grandes')
        ],
        pasos: [
          LocalizedString(gl: 'Tocar a auga fresca', es: 'Tocar el agua fresca')
        ],
      ...
      matematicas: MatematicasTempras(
        ...
        accionesSugeridas: [
          LocalizedString(gl: 'Separar conchas', es: 'Separar conchas')
        ],
      ...
      puenteCasa: PonteCasa(
        ...
        actividadesSugeridas: [
          LocalizedString(
              gl: 'Mirar o mar no paseo', es: 'Mirar el mar en el paseo')
        ],
  ...
  group('Juega con Lúa Feature Tests', () {
    ...
    testWidgets(
        'AsambleaGuiadaScreen navigates through all 6 phases and manages audio',
        (tester) async {
      await tester.pumpWidget(
        MaterialApp(
      ...
      expect(find.text('Nivel 1: Sinalar e identificación visual'),
          findsOneWidget);
      expect(find.text('Nivel 2: Nomear e onomatopeias'), findsOneWidget);
      expect(find.text('Nivel 3: Causa-efecto e experiencia cotiá'),
          findsOneWidget);
      ...
      expect(
          find.text('Ponte á casa: Comunicación con familias'), findsOneWidget);
