...
void main() {
  group('1. Enums and Canonical Phase Specifications', () {
    ...
    test(
        'TipoFaseAsamblea enforces exact 4 canonical durations summing to 600s',
        () {
      expect(TipoFaseAsamblea.aperturaSaudo.orden, equals(1));
      expect(
          TipoFaseAsamblea.aperturaSaudo.duracionCanonicoSegundos, equals(90));
      expect(
          TipoFaseAsamblea.aperturaSaudo.duracionMinutosDecimal, equals(1.5));
      ...
      expect(TipoFaseAsamblea.movementRhythmFocus.duracionCanonicoSegundos,
          equals(120));
      expect(TipoFaseAsamblea.movementRhythmFocus.duracionMinutosDecimal,
          equals(2.0));

      expect(TipoFaseAsamblea.coreTprChallenge.orden, equals(3));
      expect(TipoFaseAsamblea.coreTprChallenge.duracionCanonicoSegundos,
          equals(270));
      expect(TipoFaseAsamblea.coreTprChallenge.duracionMinutosDecimal,
          equals(4.5));
      ...
      expect(totalSeconds, equals(600),
          reason: 'Assembly duration must sum to exactly 600s (10 min)');

      expect(
          TipoFaseAsamblea.porOrden(1), equals(TipoFaseAsamblea.aperturaSaudo));
  ...
  group('2. Component Models Serialization and Invariants', () {
    ...
    test('FaseAsamblea exposes formatted duration and canonical accessors', () {
      ...
      expect(fase.comandos.first.textoIngles,
          equals('Walk to the circle and sit down'));
    ...
    test(
        'CurricularReferenceSegundoCiclo validates Decreto 150/2022 areas and criteria',
        () {
      final json = {
    ...
    test(
        'MicroRutinaHogarSegundoCiclo and PautaRecast model home Time & Place (3-5 min)',
        () {
      final json = {
        'id': 'micro.percha.01',
        'titulo': {
        ...
        'pautasRecast': [
          {
            ...
            'modeladoIndirecto': {
              'gl':
                  'Ai, o abrigo quere durmir na percha! "Up on the hook, zip!"',
              'es':
                  'Ay, el abrigo quiere dormir en la percha! "Up on the hook, zip!"'
            },
            'consejoEvitar': {
  ...
  group('3. Deserialization and Serialization Round-Trip of 4.º, 5.º, and 6.º Models', () {
            ...
            'repertorioMateriales': [
              {
                ...
                'avisoSeguridad': {
                  'gl': 'Dimensión 30x30 cm',
                  'es': 'Dimensión 30x30 cm'
                },
              }
            ],
          }
    ...
    test(
        'Round-trip 5.º Infantil: Dramatized & Narrative TPR with Backpack Story and Stop-Signal',
        () {
      final json5i = {
        ...
        'fases': [
          ...
          },
          {
            ...
            'tipo': 'movement_rhythm_focus',
            'titulo': {
              'gl': 'Foco Rítmico e Praxias',
              'es': 'Foco Rítmico y Praxias'
            },
            'duracionSegundos': 120,
            'consignaDocente': {
              'gl':
                  'Rimas dactilares e sons orofaciais: Pitter-patter rain, click-clack shoes',
              'es':
                  'Rimas dactilares y sonidos orofaciales: Pitter-patter rain, click-clack shoes'
            },
            'comandosL3': [],
            'repertorioMateriales': [],
          },
          {
            ...
            'tipo': 'core_tpr_challenge',
            'titulo': {
              'gl': 'Micro-narrativa da mochila e Freeze',
              'es': 'Micro-narrativa de la mochila y Freeze'
            },
            'duracionSegundos': 270,
            'consignaDocente': {
            ...
            'repertorioMateriales': [
              {
                'id': 'castanas_vigo',
                'nombre': {
                  'gl': 'Castañas autóctonas',
                  'es': 'Castañas autóctonas'
                },
                'procedencia': {'gl': 'Soutos galegos', 'es': 'Sotos gallegos'},
                'pautaManipulacion': {
                  'gl': 'Rodar polas palmas',
                  'es': 'Rodar por las palmas'
                },
                'avisoSeguridad': {
                  'gl': 'Diámetro > 4.0 cm',
                  'es': 'Diámetro > 4.0 cm'
        ...
        'microRutinaHogar': {
          ...
          'momentoDelDia': {
            'gl': 'Noite antes de durmir',
            'es': 'Noche antes de dormir'
          },
          'objetivoAutonomia': {
            'gl': 'Gardar a botella de auga',
            'es': 'Guardar la botella de agua'
          },
          'pautasRecast': [
            {
              'expresionMenor': {
    ...
    test(
        'Round-trip 6.º Infantil: Transactional TPR with Peer-to-Peer and Scallop Shells',
        () {
      final json6i = {
        ...
        'fases': [
          {
            ...
            'duracionSegundos': 90,
            'consignaDocente': {
              'gl': 'High five en parellas',
              'es': 'High five en parejas'
            },
            'comandosL3': [],
            'repertorioMateriales': [],
          },
          {
            ...
            'tipo': 'movement_rhythm_focus',
            'titulo': {
              'gl': 'Compás Rítmico Cruzado',
              'es': 'Compás Rítmico Cruzado'
            },
            'duracionSegundos': 120,
            'consignaDocente': {
              'gl': 'Palmas cruzadas entre iguais',
              'es': 'Palmas cruzadas entre iguales'
            },
            'comandosL3': [],
            'repertorioMateriales': [],
          },
          {
            ...
            'tipo': 'core_tpr_challenge',
            'titulo': {
              'gl': 'Reto Transaccional con Tarxeta Icónica',
              'es': 'Reto Transaccional con Tarjeta Icónica'
            },
            'duracionSegundos': 270,
            'consignaDocente': {
              'gl':
                  'A crianza A amosa a tarxeta sen texto e a crianza B executa o comando',
              'es':
                  'El menor A muestra la tarjeta sin texto y el menor B ejecuta el comando'
            },
            'comandosL3': [
              {
                'id': 'cmd.6i.01',
                ...
                'accionFisica': {
                  'gl':
                      'Guiar ao compañeiro ata a percha amarela e colgar o abrigo',
                  'es':
                      'Guiar al compañero hasta la percha amarilla y colgar el abrigo'
                },
                'modeladoDocente': {
          ...
          },
          {
            ...
            'tipo': 'calma_transicion',
            'titulo': {
              'gl': 'Calma con Cunchas da Ría de Vigo',
              'es': 'Calma con Conchas de la Ría de Vigo'
            },
            'duracionSegundos': 120,
            'consignaDocente': {
              'gl':
                  'Escoitar o eco do mar coas cunchas e deixalas no cesto de vimbio',
              'es':
                  'Escuchar el eco del mar con las conchas y dejarlas en el cesto de mimbre'
            },
            'comandosL3': [],
            'repertorioMateriales': [
              {
                'id': 'cunchas_ria_vigo',
                'nombre': {
                  'gl': 'Cunchas de vieira da Ría de Vigo',
                  'es': 'Conchas de vieira de la Ría de Vigo'
                },
                'procedencia': {
                  'gl': 'Praias e bateas da ría',
                  'es': 'Playas y bateas de la ría'
                },
                'pautaManipulacion': {
                ...
                'avisoSeguridad': {
                  'gl':
                      'Diámetro > 5.0 cm, esterilizadas e sen bordos cortantes',
                  'es':
                      'Diámetro > 5.0 cm, esterilizadas y sin bordes cortantes'
                }
              }
            ],
          }
          ...
          'pautasRecast': [
            {
              'expresionMenor': {
                'gl': 'Zapatos gardados!',
                'es': 'Zapatos guardados!'
              },
              'modeladoIndirecto': {
                'gl': 'Que ben! Gardaches os zapatos no andel azul ti soa!',
                'es':
                    '¡Qué bien! ¡Has guardado los zapatos en el estante azul tú sola!'
              },
              'consejoEvitar': {
                'gl': 'Evitar esixencia de rapidez',
                'es': 'Evitar exigencia de rapidez'
              }
            }
          ],
      ...
      expect(
          asamblea.metodologia, equals(MetodologiaTPR.transaccionalPragmatico));
      ...
      expect(
          fase4.materiaisNaturais.first.avisoSeguridad!.gl, contains('5.0 cm'));
  ...
  group('4. Invariant Enforcement and Edge Cases Tests', () {
    ...
    test('handles missing or blank optional fields defensively without throwing',
      ...
      expect(asamblea.microRutinaHogar, isNotNull);
    });
  });
  ...
  group('5. ContentAssetLoader and ContentRepository Extensions for Segundo Ciclo', () {
    test(
      ...
      final asamblea = loader.parseAsambleaSegundoCiclo(validJson);
      expect(asamblea.id, equals('asamblea.test.loader'));
      expect(asamblea.nivel, equals(NivelEducativoSegundoCiclo.infantil4));

      expect(
        () => loader.parseAsambleaSegundoCiclo('["invalid_list_root"]'),
        throwsFormatException,
      );
    });

    test(
        'ContentRepository manages Segundo Ciclo assemblies in-memory and query methods',
        () async {
      final repo = ContentRepository();
      expect(repo.asambleaSegundoCicloCount, equals(0));

      final a4 = AsambleaSegundoCiclo.fromJson({
        'id': 'asamblea.setembro.4_infantil',
        'nivel': '4_infantil',
        'mes': 9,
        'titulo': {'gl': 'Setembro 4', 'es': 'Septiembre 4'},
        'centroInteres': {'gl': 'C', 'es': 'C'},
        'metodologiaTpr': 'accion_expandida',
        'duracionTotalMinutos': 10,
        'fases': [],
      ...
      final a5 = AsambleaSegundoCiclo.fromJson({
        'id': 'asamblea.setembro.5_infantil',
        'nivel': '5_infantil',
        'mes': 9,
        'titulo': {'gl': 'Setembro 5', 'es': 'Septiembre 5'},
        'centroInteres': {'gl': 'C', 'es': 'C'},
        'metodologiaTpr': 'dramatizado_narrativo',
        'duracionTotalMinutos': 10,
        'fases': [],
      ...
      repo.addAsambleaSegundoCiclo(a4);
      repo.addAsambleaSegundoCiclo(a5);

      expect(repo.asambleaSegundoCicloCount, equals(2));
      expect(repo.isInitialized, isTrue);

      // Async queries
      final all = await repo.getAllAsambleasSegundoCiclo();
      expect(all.length, equals(2));

      final byId = await repo
          .getAsambleaSegundoCicloById('asamblea.setembro.4_infantil');
      expect(byId, equals(a4));
      ...
      final byMesNivel = await repo.getAsambleaByMesYNivel(
          9, NivelEducativoSegundoCiclo.infantil5);
      expect(byMesNivel, equals(a5));

      final byMesNivelMissing = await repo.getAsambleaByMesYNivel(
          10, NivelEducativoSegundoCiclo.infantil5);
      expect(byMesNivelMissing, isNull);
      ...
      expect(
          repo
              .getAsambleasByNivelSync(NivelEducativoSegundoCiclo.infantil5)
              .length,
          equals(1));
      expect(
