...
void main() {
  ...
  group('Curricular Alignment Tests (Decreto 150/2022 do 8 de setembro)', () {
    test('certifies curricular alignment for juega.mar.01.json', () {
      ...
      expect(errors, isEmpty, reason: 'Curricular validation failed: ${errors.join(", ")}');
      ...
      expect(ref.hasArea(CurricularReference.area2DescubrimentoContorna), isTrue);
      expect(ref.hasArea(CurricularReference.area3ComunicacionRepresentacion), isTrue);

      // Criteria check
      expect(ref.hasCriterio(CurricularReference.criterioExploracionMateriais), isTrue); // CA2.1
      expect(ref.hasCriterio(CurricularReference.criterioRazoamentoElementar), isTrue);  // CA2.2
      expect(ref.hasCriterio(CurricularReference.criterioComunicacionAfectiva), isTrue); // CA3.1
      expect(ref.hasCriterio(CurricularReference.criterioLecturaCompartida), isTrue);   // CA3.2
    });

    test('certifies curricular alignment for academy.como_se_aprende_a_hablar.01.json', () {
      ...
      expect(ref.hasArea(CurricularReference.area3ComunicacionRepresentacion), isTrue);
      expect(ref.hasCriterio(CurricularReference.criterioCuriosidadeSeguridade), isTrue); // CA1.1
      expect(ref.hasCriterio(CurricularReference.criterioComunicacionAfectiva), isTrue);  // CA3.1
    });

    test('rejects foreign or outdated regulation (e.g. Decreto 330/2009 or LOMLOE generic)', () {
      final oldRegulation = {
