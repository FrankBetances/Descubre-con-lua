...
void main() {
  ...
  group('ContentValidator placeholderPattern Case Sensitivity Tests', () {
    test('allows legitimate usage of lowercase "todo" and capitalized "Todo" (Spanish/Galician for "all")', () {
      final validSamples = [
        {'gl': 'e, sobre todo, dan contexto', 'es': 'y, sobre todo, dan contexto'},
        {'gl': 'Todo o alumnado participa na asemblea', 'es': 'Todo el alumnado participa en la asamblea'},
        {'gl': 'Recollemos todo o material con agarimo', 'es': 'Recogemos todo el material con cariño'},
        {'gl': 'Facemos as actividades todos xuntos', 'es': 'Hacemos las actividades todos juntos'},
        {'gl': 'Hai de todo no recanto dos xogos', 'es': 'Hay de todo en el rincón de los juegos'},
        {'gl': 'Acollida con todo o cariño', 'es': 'Acogida con todo el cariño'},
        {'gl': 'O método pedagóxico é activo', 'es': 'El método pedagógico es activo'},
      ];
      ...
      for (final sample in validSamples) {
        ...
        expect(
          errors,
          isEmpty,
          reason: 'Validator falsely flagged valid text containing "todo": "${sample['gl']}"',
        );
      }
    });

    test('strictly rejects developer placeholder markers (TODO, TBD, PLACEHOLDER, PENDIENTE, PENDENTE, LOREM IPSUM)', () {
      ...
      for (final placeholder in placeholderSamples) {
        ...
        expect(
          errors,
          isNotEmpty,
          reason: 'Validator failed to reject forbidden placeholder: "$placeholder"',
        );
    ...
    test('direct RegExp pattern assertions for placeholderPattern', () {
      ...
      expect(ContentValidator.placeholderPattern.hasMatch('Todo'), isFalse);
      expect(ContentValidator.placeholderPattern.hasMatch('sobre todo'), isFalse);
      ...
      expect(ContentValidator.placeholderPattern.hasMatch('placeholder'), isFalse);
      expect(ContentValidator.placeholderPattern.hasMatch('pendiente'), isFalse);
      ...
      expect(ContentValidator.placeholderPattern.hasMatch('PLACEHOLDER'), isTrue);
      ...
      expect(ContentValidator.placeholderPattern.hasMatch('LOREM IPSUM'), isTrue);
    });
  });
}
