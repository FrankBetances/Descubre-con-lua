...
> **Este documento describe el diseño, no el estado.** Para saber qué funciona
> y qué no, con la evidencia al lado, lee [`STATUS.md`](STATUS.md). Las cuatro
> milestones figuraron como DONE mientras la app no compilaba.
...
- `lib/core/`:
  ...
  - `audio/`: `OfflineAudioService` interface, `LocalAudioPlayer` (MethodChannel sobre el `MediaPlayer` de Android; es el que usa la app), `MockOfflineAudioService` sólo para tests, y `voice_id.dart`, que deriva la ruta de cada grabación del hash de su texto.
...
| 9 | Vigo Maritime Base Unit | `assets/content/unidades/juega.mar.01.json` (Samil, ría de Vigo, pulso 72 BPM) | M2 | Survey / R2 |
...
| 15 | Academy 5 Blocks Navigation | 5 bloques: `desarrollo_comunicativo`, `rutinas_y_bano_de_lenguaje`, `turnos_y_atencion_conjunta`, `juego_movimiento_sin_pantallas`, `bilinguismo_y_cultura` | M3 | Survey / R3 |
...
| 22 | Offline Audio Asset Bundling | Pista de pulso `assets/audio/mar_pulso_72bpm.wav`, **medida** en 72,3 BPM, más `assets/voice/*.m4a` sintetizados en CI | M3 | Survey / R3 |
...
|---|------|-------|-------------|--------|
| 1 | M1: Base Flutter Android Setup & Privacy | Scaffolding, `pubspec.yaml`, `android/` setup (`com.earlify.descubreconlua`), zero internet permission manifest, `lib/core/` and `OfflineAudioService` | none | Código presente. **APK nunca compilado** (ver STATUS.md) |
| 2 | M2: Content-as-Data, JSON Assets & Validation Suite | Dart models, `ContentAssetLoader`, `ContentRepository`, base JSONs, automated validation test suite (`test/data/`) | M1 | Tests verdes, ejecutados de verdad |
| 3 | M3: Pedagogical Modules (Academy & Juega con Lúa) | Academy 5 blocks & 4-part capsule screens, Juega con Lúa 6-step assembly flow with offline audio controller, offline audio asset bundling | M2 | Pantallas presentes; **ninguna vista en un aparato**; sin grabaciones |
| 4 | M4: Comprehensive Verification & E2E Testing | `tools/gates.sh` y CI ejecutando la suite real, build de release y auditoría de permisos del APK | M3 | Gates en su sitio; 1 falla a propósito (sin grabaciones) |
